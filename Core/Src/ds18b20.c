...
{
  uint16_t status;
	GPIOB->ODR &= ~GPIO_ODR_ODR10;//низкий уровень
  DelayMicro(485);//задержка как минимум на 480 микросекунд
  GPIOB->ODR |= GPIO_ODR_ODR10;//высокий уровень
  DelayMicro(65);//задержка как минимум на 60 микросекунд
  status = GPIOB->IDR & GPIO_IDR_IDR10;//провер€ем уровень
...
uint8_t ds18b20_ReadBit(void)
{
  ...
  DelayMicro(2);
	GPIOB->ODR |= GPIO_ODR_ODR10;//высокий уровень
	DelayMicro(13);
	bit = (GPIOB->IDR & GPIO_IDR_IDR10 ? 1 : 0);//провер€ем уровень	
	DelayMicro(45);
  return bit;
}
//-----------------------------------------------
